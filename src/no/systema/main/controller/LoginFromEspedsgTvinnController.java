...
/**
 * Login for the external module: espedsgtvinnsad
 * 
 * To accomplish direct link into whatever tror - view (JSP). 
...
@Controller
@Scope("session")
public class LoginFromEspedsgTvinnController {
	private static final Logger logger = Logger.getLogger(LoginFromEspedsgTvinnController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="logonTvinnSad_toTror.do", method= { RequestMethod.POST, RequestMethod.GET })
	...
	@RequestMapping(value="logonWRedTvinnSad_toTror.do", method= { RequestMethod.POST, RequestMethod.GET})
	...
	private ModelAndView getSuccessView (SystemaWebUser appUser, HttpServletRequest request){
		...
		String avd = request.getParameter("heavd");
		String opd = request.getParameter("heopd");
		String signatur = request.getParameter("hesg");
		...
		logger.info("TROR opd EXISTS !");
		successViewStr = "redirect:tror_mainorderlist.do?action=doFind" + "&avd=" + avd + "&orderNr=" + opd + "&sign=" + signatur;
	...
	private String getTomcatServerRedirectionUrl(SystemaWebUser appUser, HttpServletRequest request){
		...
		try{
			...
			params.append("/logonWRedTvinnSad_toTror.do?user=" + appUser.getUser());
