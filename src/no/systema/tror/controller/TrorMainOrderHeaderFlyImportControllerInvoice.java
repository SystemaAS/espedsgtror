...
public class TrorMainOrderHeaderFlyImportControllerInvoice {
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyimport_invoice_recalculate.do", method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doRecalculateInvoiceLines(@ModelAttribute ("record") JsonTrorOrderHeaderRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("redirect:tror_mainorderflyimport_invoice.do?action=doInit&heavd=" + recordToValidate.getHeavd() + "&heopd=" + recordToValidate.getHeopd());
		SystemaWebUser appUser = this.loginValidator.getValidUser(session);
		String parentTrip = recordToValidate.getHepro();
