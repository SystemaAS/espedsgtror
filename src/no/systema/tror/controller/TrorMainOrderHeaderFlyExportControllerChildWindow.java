...
public class TrorMainOrderHeaderFlyExportControllerChildWindow {
	
	private static final Logger logger = Logger.getLogger(TrorMainOrderHeaderFlyExportControllerChildWindow.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyexport_childwindow_airlines.do", params="action=doFind",  method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doFindAirlines(@ModelAttribute ("record") Sted2Dao recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		Collection outputList = new ArrayList();
		Map model = new HashMap();
		ModelAndView successView = new ModelAndView("tror_mainorderflyexport_childwindow_airlines");
		SystemaWebUser appUser = this.loginValidator.getValidUser(session);
		//check user (should be in session already)
		if(appUser==null){
			return loginView;
	...
	*/
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyexport_childwindow_incoterms.do", params="action=doFind",  method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doFindIncoterms(@ModelAttribute ("record") KodtfrDao recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		Collection outputList = new ArrayList();
		Map model = new HashMap();
		ModelAndView successView = new ModelAndView("tror_mainorderflyexport_childwindow_incoterms");
		SystemaWebUser appUser = this.loginValidator.getValidUser(session);
		//check user (should be in session already)
		if(appUser==null){
			return loginView;
