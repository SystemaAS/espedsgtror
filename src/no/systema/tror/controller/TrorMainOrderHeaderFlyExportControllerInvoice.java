...
@Scope("session")
public class TrorMainOrderHeaderFlyExportControllerInvoice {
	private static final JsonDebugger jsonDebugger = new JsonDebugger(1500);
	private static Logger logger = Logger.getLogger(TrorMainOrderHeaderFlyExportControllerInvoice.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyexport_invoice.do", method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doGetInvoice(@ModelAttribute ("record") JsonTrorOrderHeaderRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("tror_mainorderflyexport_invoice");
		...
		if(appUser==null){
			...
			if(recordToValidate.getHeavd()!=null && !"".equals(recordToValidate.getHeavd()) && 
	    		...
	    		if(jsonPayload!=null){
	    			JsonTrorOrderFlyExportInvoiceContainer container = this.trorMainOrderHeaderFlyexportService.getOrderInvoiceContainer(jsonPayload);
	    			if(container!=null){
	    				//ready mark status
	    				JsonTrorOrderFlyExportInvoiceReadyMarkContainer readyMarkContainer = this.getReadyMark(appUser, recordToValidate.getHeavd(), recordToValidate.getHeopd());
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyexport_invoice_recalculate.do", method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doRecalculateInvoiceLines(@ModelAttribute ("record") JsonTrorOrderHeaderRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("redirect:tror_mainorderflyexport_invoice.do?action=doInit&heavd=" + recordToValidate.getHeavd() + "&heopd=" + recordToValidate.getHeopd());
		SystemaWebUser appUser = this.loginValidator.getValidUser(session);
		String parentTrip = recordToValidate.getHepro();
	 ...
	 * @return
	 */
	private   JsonTrorOrderFlyExportInvoiceReadyMarkContainer getReadyMark(SystemaWebUser appUser, String avd, String opd ){
		JsonTrorOrderFlyExportInvoiceReadyMarkContainer retval = null;
		...
		if(jsonPayload!=null){
			...
			JsonTrorOrderFlyExportInvoiceReadyMarkContainer container = new JsonTrorOrderFlyExportInvoiceReadyMarkContainer();
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyexport_invoice_edit.do",  method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doEditInvoice(@ModelAttribute ("record") JsonTransportDispWorkflowSpecificOrderInvoiceRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("redirect:tror_mainorderflyexport_invoice.do?action=doFind&heavd=" + heavd + "&heopd=" + heopd + "&itemsType=O");
		ModelAndView errorView = new ModelAndView("tror_mainorderflyexport_invoice");
	 ...
	 * @param session
	 */
	private void setDomainObjectsInView(Map model, JsonTrorOrderFlyExportInvoiceContainer container, HttpSession session){
		List<JsonTrorOrderFlyExportInvoiceRecord> list = new ArrayList<JsonTrorOrderFlyExportInvoiceRecord>();
		...
		Collections.sort(list, new JsonTrorOrderFlyExportInvoiceRecord.OrderByCodeAndInvoiceNr());
		if(list!=null){
		...
		//DEBUG
		/*for(JsonTrorOrderFlyExportInvoiceRecord record : list){
	 ...
	 * @return
	 */
	private List<JsonTrorOrderFlyExportInvoiceRecord> getListWithTotals(Map<Integer, List<JsonTrorOrderFlyExportInvoiceRecord>> listGroupsMap){
		List<JsonTrorOrderFlyExportInvoiceRecord> listWithTotals = new ArrayList<JsonTrorOrderFlyExportInvoiceRecord>();
		int TWO_DECIMALS = 2;
	    ...
	    while (it.hasNext()) {
	        ...
	        List<JsonTrorOrderFlyExportInvoiceRecord> group = (List<JsonTrorOrderFlyExportInvoiceRecord>)pair.getValue();
	        Double fablnTotal = 0.00D;
	        for (JsonTrorOrderFlyExportInvoiceRecord groupRecord : group){
	 ...
	 * @return
	 */
	private Map<Integer,List<JsonTrorOrderFlyExportInvoiceRecord>> setListGroups(List<JsonTrorOrderFlyExportInvoiceRecord> list){
		Map<Integer,List<JsonTrorOrderFlyExportInvoiceRecord>> map = new HashMap<Integer, List<JsonTrorOrderFlyExportInvoiceRecord>>();
		List<JsonTrorOrderFlyExportInvoiceRecord> newList = new ArrayList<JsonTrorOrderFlyExportInvoiceRecord>();
		String previousCode = "";
		int index = 0;
		Integer mapIndex = 0;
		for (JsonTrorOrderFlyExportInvoiceRecord record: list){
			...
			if (index==0){
				...
				if (previousCode.equals(record.getFask() + record.getFafakt())){
					...
					mapIndex++;
					newList = new ArrayList<JsonTrorOrderFlyExportInvoiceRecord>();
					newList.add(record);
					map.put(mapIndex, newList);
				}
				previousCode = record.getFask() + record.getFafakt();
	 ...
	 * @return
	 */
	private JsonTrorOrderFlyExportInvoiceRecord addTotalRecord(){
		JsonTrorOrderFlyExportInvoiceRecord record = new JsonTrorOrderFlyExportInvoiceRecord();
