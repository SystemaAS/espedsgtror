	...
	jq(function() {
	  	...
	  	//by beregning button
	  	  jq('#recalculateButton').click(function() {
	  		  jq.blockUI({ message: BLOCKUI_OVERLAY_MESSAGE_DEFAULT});
	  		  window.location = "tror_mainorderflyexport_invoice_recalculate.do?action=doInit&heavd=" + jq('#heavd').val() + "&heopd=" + jq('#heopd').val();
	  ...
	  jq('<div></div>').dialog({
        ...
        buttons: {
	        Fortsett: function() {
	            ...
	            //do delete
	            jq.blockUI({ message: BLOCKUI_OVERLAY_MESSAGE_DEFAULT});
	            window.location = "tror_mainorderflyexport_invoice_edit.do?action=doDelete" + "&heavd=" + heavd + "&heopd=" + heopd + "&fali=" + fali;
	        },
	        Avbryt: function() {
	            jq( this ).dialog( "close" );
	        }
